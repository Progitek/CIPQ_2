...
//messagebox('isvalide',isvalid(w_fichierxml))
//gnv_app.inv_entrepotglobal.of_ajoutedonnee( "bon_modification", string(ib_change))
end event
...
event ue_buttonclicked;call super::ue_buttonclicked;CHOOSE CASE as_button
	...
	CASE "Enregistrer"
		...
		if ib_change then
			ll_row_dw = dw_bon_expedition.GetRow()
			IF ll_row_dw > 0 THEN
				ls_cie_no = dw_bon_expedition.object.cie_no[ll_row_dw]
				ls_liv_no = dw_bon_expedition.object.liv_no[ll_row_dw]
				select count(*) into :ll_existe
				from t_imgcodeqr
				where liv_no = :ls_liv_no and cie_no = :ls_cie_no;
				if ll_existe > 0 then
					update t_imgcodeqr set mod = 1 
					where liv_no = :ls_liv_no and cie_no = :ls_cie_no;
