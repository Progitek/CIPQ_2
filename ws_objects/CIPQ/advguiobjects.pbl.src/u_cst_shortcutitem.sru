...
long ll_grosseur
//ll_grosseur = long(gnv_app.inv_options.of_recuperer_option("grosseur_font_toolbarstrip"))
IF IsNull(ll_grosseur) THEN ll_grosseur = 11

IF iuo_parent.il_CurrentSize = iuo_parent.SMALL THEN
	in_Gradient.of_DrawText(THIS, is_text, iuo_parent.of_GetColor('ITEMTEXT'), 'Tahoma', ll_grosseur, TRUE, in_Gradient.ALIGN_LEFT, 130, 36, Width - 120, 64,iuo_parent.of_ShadowText()) 
ELSE
	in_Gradient.of_DrawText(THIS, is_text, iuo_parent.of_GetColor('ITEMTEXT'), 'Tahoma', ll_grosseur, TRUE, in_Gradient.ALIGN_LEFT, 160, 40, Width - 120, 64,iuo_parent.of_ShadowText()) 
END IF
