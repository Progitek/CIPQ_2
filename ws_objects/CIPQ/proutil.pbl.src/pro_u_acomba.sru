...
private function long uf_getprojectcardpos (string as_nopro)
public function integer uf_ajoutfact (string as_nofact, string as_desc, string as_noclient, string as_noprojet, date adt_date, str_lignefact astr_lignes[], ref string as_accnumfact, ref string as_erreur)
end prototypes
...
private function long uf_getprojectcardpos (string as_nopro);OLEObject lobj_ProjectIndex
long ll_ret

if isNull(iobj_Acomba) or not isValid(iobj_Acomba) then return 0

lobj_ProjectIndex = create OLEObject

if lobj_ProjectIndex.ConnectToNewObject("AcoX0127.ProjectKey") <> 0 then
	destroy lobj_ProjectIndex
	return 0
end if

//Initialiser la propriété CuNumber
lobj_ProjectIndex.PrNumber = as_nopro

//Recherche du client dans l'index 1 pour obtenir son CardPos
if lobj_ProjectIndex.XFindKey(1) = 0 then
	ll_ret = lobj_ProjectIndex.PrCardPos
...
public function integer uf_ajoutfact (string as_nofact, string as_desc, string as_noclient, string as_noprojet, date adt_date, str_lignefact astr_lignes[], ref string as_accnumfact, ref string as_erreur);OLEObject lobj_ControlCustomerData
...
iobj_TransactionData.InShipCareOf = as_nofact
iobj_TransactionData.InDescription = as_desc
iobj_TransactionData.InDate = adt_date
...
//Numéro du projet associé à la facture
iobj_TransactionData.InProjectCP = uf_GetProjectCardPos(as_NoProjet)
...
for i = lowerbound(astr_lignes) to upperbound(astr_lignes)
	//Informations générales pour chaque ligne de produit
	li_cpt ++
	iobj_TransactionData.ILType[li_cpt] = astr_lignes[i].i_type
	iobj_TransactionData.ILProjectCP[li_cpt] = iobj_TransactionData.InProjectCP
...
as_accnumfact = iobj_TransactionData.InInvoiceNumber
