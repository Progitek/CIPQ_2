$PBExportHeader$w_impression_etiquette.srw
forward
...
// S'il y a une imprimante par défaut, la sélectionner
...
choose case dwo.name
	...
	case "nb_pages"
		IF IsNull(data) THEN
			this.object.st_page.text = "Aucune étiquette"
		ELSE
			ll_nbpages = long(data) * 22
			
			this.object.st_page.text = "Pages pour : " + string(ll_nbpages) + " étiquettes"
		END IF		
...
string text = "Impression d~'étiquettes"
boolean focusrectangle = false
end type
...
//Changé pour impression directe
