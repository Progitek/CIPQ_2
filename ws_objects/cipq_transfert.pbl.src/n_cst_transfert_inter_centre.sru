$PBExportHeader$n_cst_transfert_inter_centre.sru
forward
...
string	ls_retour, ls_prefnom, ls_cie, ls_nocommande, ls_ciecentre, ls_repexport, ls_NameInit, ls_ecrire = '', &
			ls_string, ls_app = "'", ls_sql, ls_strtmp, ls_newnocommande, ls_newnocommandetab[], ls_oldnocommandetab[]
...
dec		ldec_QteInit, ldec_QteCommande, ldec_QteExpedie, ldec_QteTransfert, ldec_QteTransfertTot, j, x, ll_retour, u, ll_index
integer	li_FileNum
datetime	ldt_heure
string ls_tab[], ls_odbc, ls_stratraiter, ls_dernumero, ls_ciebase
...
IF gnv_app.inv_error.of_message("CIPQ0078") = 1 THEN
	...
	// Vérifier si une commande qu'on s'apprête à transférer est traitée
	setNull(ll_eleveur)
	...
	if SQLCA.SQLCode = -1 then
		ls_sql = "create table #TMP_Trans_Commande (CieNo varchar(3) not null,~r~n" + &
																 "NoCommande varchar(8) not null,~r~n" + &
																 "No_Eleveur integer null,~r~n" + &
																 "NoBonExpe varchar(7) null,~r~n" + &
																 "BonCommandeClient varchar(8) null,~r~n" + &
	...
	//Déterminer les centres transférables
	...
	FOR ll_cpt_centre = 1 TO ll_row_transferable
		...
		//Vérifier si éléments à transférer pour ce centre, pour cette date
		ll_rowcount_a = lds_commandes_a_transferer.Retrieve(ad_cur, ls_prefnom)
		
		FOR ll_cpt_commande = 1 TO ll_rowcount_a
			
			//Préparer les tables qui serviront pour générer les commandes SQL
         ...
         WHERE t_Commande.CieNo = :ls_cie AND t_Commande.NoCommande = :ls_nocommande;
			...
			//les inscriptions à imprimer batch
			ls_ecrire = ls_ecrire + THIS.of_dosavetoprinter(ls_cie, ls_nocommande, ls_ciecentre) 
			ls_stratraiter = ls_stratraiter + THIS.of_dosavetoprinter(ls_cie, ls_nocommande, ls_ciecentre) 
			
			//On vérifie les qtés commandées, expédiées et transférées
			SELECT 
			SUM(QteInit),
			SUM(QteCommande),
			SUM(QteExpedie),
			...
			//MAJ des qté initiale, commandé et transféré
			UPDATE 	#TMP_Trans_CommandeDetail 
			...
			//Si toute la commande est transférée: (le total des QtéInit= au total des qtés transféré) 
			// et (le total des qtés en commande=0) on va la considérer comme traitée et comme imprimée
			// et ne sera jamais facturée
			If ldec_QteInit = ldec_QteTransfertTot Then
				UPDATE t_Commande SET t_Commande.Traiter = 1, t_Commande.Imprimer = 1 
				WHERE t_Commande.CieNo = :ls_cie AND t_Commande.NoCommande = :ls_nocommande ;
				IF SQLCA.SQLCode <> 0 then
					gnv_app.inv_error.of_message("CIPQ0152",{"Commande toute transférée", SQLCA.SQLeRRText})
				ELSE
					COMMIT USING SQLCA;
					IF SQLCA.SQLCode <> 0 then
						gnv_app.inv_error.of_message("CIPQ0152",{"Commande toute transférée", SQLCA.SQLeRRText})
		...
		//Les données sont préparées, on prépare l'export
		
		
		//Ajouter les commandes
		lds_Tmplt_Commande.Retrieve()
		//Le saveas écrase le fichier et ajoute une entete avec le create
		lds_Tmplt_Commande.SaveAs(gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", SQLInsert!, FALSE)
		ls_string = ""
		DO WHILE YIELD()
		LOOP
		IF FileExists(gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt") THEN
			li_FileNum = FileOpen(gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", TextMode!)
			IF li_FileNum = -1 THEN
				gnv_app.inv_error.of_message("CIPQ0151",{"Ouverture de fichier", gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", "of_transfert 1 (inter-centre)"})
			END IF			
		...
		ELSE
			gnv_app.inv_error.of_message("CIPQ0153", { gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", "of_transfert B" })
		END IF
		ls_ecrire = ls_ecrire + "~r~n" + ls_string + " "
		ls_stratraiter = ls_stratraiter + ls_string + " "
				
		//Ajouter les commandes détail
		...
		IF ll_rowcount_temp = 0 AND ll_rowcount_a > 0 THEN
			Messagebox("Attention", "Il y a des commandes à transférer mais il n'y a pas de détail. Cette situation est irrégulière et il se peut que ces commandes ne se soient pas transférées correctement.")
		END IF
		
		//Le saveas écrase le fichier et ajoute une entete avec le create
		lds_Tmplt_CommandeDetail.SaveAs(gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", SQLInsert!, FALSE)
		ls_string = ""
		DO WHILE YIELD()
		LOOP
		...
		IF FileExists(gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt") THEN
			li_FileNum = FileOpen(gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", TextMode!)
			IF li_FileNum = -1 THEN
				gnv_app.inv_error.of_message("CIPQ0151",{"Ouverture de fichier",gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", "of_transfert (inter-centre)"})
			END IF
		...
		ELSE
			gnv_app.inv_error.of_message("CIPQ0153", {gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", "of_transfert C" })
		END IF
		ls_ecrire = ls_ecrire + "~r~n" + ls_string + " "
		ls_stratraiter = ls_stratraiter + ls_string + " "
		...
		IF trim(ls_ecrire) <> "" AND TRIM(gnv_app.inv_string.of_globalreplace( ls_ecrire, "~r~n", "")) <> "" THEN
			...
			IF ll_rtn < 0 THEN
				gnv_app.inv_error.of_message("CIPQ0151",{"Écriture du fichier de transfert", ls_repexport + "TMP_" +ls_NameInit, "of_transfert (inter-centre)"})
			ELSE
				//Renommer le fichier
				ll_rtn = gnv_app.inv_filesrv.of_filerename( ls_repexport + "TMP_" + ls_NameInit, ls_repexport + ls_NameInit)
				IF ll_rtn < 0 THEN
					gnv_app.inv_error.of_message("CIPQ0151",{"Écriture du fichier de transfert - rename", ls_repexport + ls_NameInit, "of_transfert (inter-centre)"})
				END IF
			END IF
		END IF
	...
	//	ls_ciebase = gnv_app.of_getcompagniedefaut( )
		...
		SELECT 	derniernocommande INTO :ls_dernumero FROM t_centreCIPQ WHERE cie = :ls_ciecentre using CIPQTRANS;
		if isnull(ls_dernumero) then 
			ls_dernumero  = '0'
		else
			ll_retour = long(ls_dernumero) - 1
			ls_dernumero = string(ll_retour)
		end if
		
		update t_centrecipq set transfert = 1 where cie = :ls_ciecentre using CIPQTRANS;
		commit using CIPQTRANS;
				
		ls_tab = split(ls_ecrire,"~r~n")
		x = 1
		
		for j = 1 to upperbound(ls_tab)
					
			if upper(left(trim(ls_tab[j]),6)) = "INSERT" or  upper(left(trim(ls_tab[j]),6)) = "UPDATE"  or left(trim(ls_tab[j]),6) = "DELETE" then
						
				if pos(ls_tab[j],"t_commande") > 0 then
					
					ll_pos = pos(ls_tab[j],",")
					ls_strtmp = mid(ls_tab[j], ll_pos + 1)
					ll_pos = pos(ls_strtmp ,",")
					ls_strtmp = left(ls_strtmp ,ll_pos - 1)
					ll_pos = pos(ls_strtmp ,",")
					ls_strtmp  = rep(ls_strtmp,"~t","")
					ls_strtmp  = rep(ls_strtmp,"'","")
					ls_strtmp  = trim(ls_strtmp)
							
					if pos(ls_tab[j],"t_commandedetail") = 0 then
						
						ll_retour = long(ls_dernumero) + 1
						ls_dernumero = string(ll_retour)
												
						ls_oldnocommandetab[x] = ls_strtmp
						ls_newnocommandetab[x] = ls_dernumero
						x++
	
					end if	
					
					for u = 1 to upperbound(ls_oldnocommandetab)
						if ls_oldnocommandetab[u] = ls_strtmp then ll_index = u
					next
					
					ls_dernumero = ls_newnocommandetab[ll_index]
						
						ls_tab[j] = rep(ls_tab[j], ls_strtmp, ls_dernumero)
					
				end if
	
				execute immediate :ls_tab[j] using CIPQTRANS;
				if CIPQTRANS.sqlcode <> 0 then
					messagebox('Query not work',CIPQTRANS.sqlerrtext)
				else
					commit using CIPQTRANS;
				end if
		...
		//		sleep(1)
				...
				update t_commandedetail set melange = null where cieno = :ls_ciecentre and nocommande = :ls_dernumero using CIPQTRANS;
				if CIPQTRANS.sqlcode <> 0 then
					messagebox('Query not work',CIPQTRANS.sqlerrtext)
				else
					commit using CIPQTRANS;
		...
		/***
		***/
//		ls_dernumero = ls_newnocommandetab[upperbound(ls_newnocommandetab)]
		ll_retour = long(ls_dernumero) + 1
		ls_dernumero = string(ll_retour)
		
		UPDATE t_centreCIPQ SET derniernocommande = :ls_dernumero WHERE	 cie = :ls_ciecentre using CIPQTRANS;
		if CIPQTRANS.sqlcode <> 0 then
			messagebox('Query not work',CIPQTRANS.sqlerrtext)
		else
			commit using CIPQTRANS;
		...
		update t_centrecipq set transfert = 0 where cie = :ls_ciecentre using CIPQTRANS;
		commit using CIPQTRANS;
		...
		//Mise à jour des éléments transférés pour ce centre
		UPDATE #TMP_Trans_CommandeDetail SET #TMP_Trans_CommandeDetail.TranName = :ls_NameInit ;
		IF SQLCA.SQLCode <> 0 then
			gnv_app.inv_error.of_message("CIPQ0152",{"Mise à jour des éléments transférés pour ce centre", SQLCA.SQLeRRText})
		ELSE
			COMMIT USING SQLCA;
			IF SQLCA.SQLCode <> 0 then
				gnv_app.inv_error.of_message("CIPQ0152",{"Mise à jour des éléments transférés pour ce centre", SQLCA.SQLeRRText})
		...
		IF SQLCA.SQLCode <> 0 then
			gnv_app.inv_error.of_message("CIPQ0152",{"2 - Mise à jour des éléments transférés pour ce centre", SQLCA.SQLeRRText})
		ELSE
			COMMIT USING SQLCA;
			IF SQLCA.SQLCode <> 0 then
				gnv_app.inv_error.of_message("CIPQ0152",{"2 - Mise à jour des éléments transférés pour ce centre", SQLCA.SQLeRRText})
		...
		ELSE
			...
			for j = 1 to upperbound(ls_oldnocommandetab)

				ls_sql = "UPDATE t_TransfertCommande SET Reception = now(), NewNoCommande = '" + ls_newnocommandetab[j] + "' WHERE CIENO = '" + ls_ciecentre + "' AND NoCommande = '" + ls_oldnocommandetab[j] + "'"
				execute immediate :ls_sql using SQLCA;
				if SQLCA.sqlcode <> 0 then
					messagebox('Query not work',SQLCA.sqlerrtext)
				end if
			next
...
IF FileExists(gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt") THEN
	FileDelete(gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt")
END IF
...
messagebox("Information!","Les commandes ont été tranférées avec succès vers le centre destinataire",Information!,Ok!)
...
IF lds_Tmplt_Commande.RowCount() > 0 THEN
	...
	ls_retour = ls_retour + "INSERT INTO Tmp_ImpressionCommande ( DescriptionCommande) VALUES (" +&
		"'Commandes transférées par : " + as_cie + "'); ~r~n"
	
	//Impression sur centre en cours
	ls_desc_to = 'Commandes transférées à: ' + as_cietransfertto 	
	...
	FOR ll_cpt_commandedetail = 1 TO lds_Tmplt_CommandeDetail.RowCount()
		...
		IF SQLCA.SQLCode <> 0 then
			gnv_app.inv_error.of_message("CIPQ0152",{"Insertion dans le backup papier - détail", SQLCA.SQLeRRText})
		ELSE
			COMMIT USING SQLCA;
			IF SQLCA.SQLCode <> 0 then
				gnv_app.inv_error.of_message("CIPQ0152",{"Insertion dans le backup papier - détail", SQLCA.SQLeRRText})
...
//On créer le fichier pour 111 dans le répertoire de transfert
ls_nomfichier = "111R" + string(month(today()),'00') + string(day(today()),'00') + & 
	string(year(today()),'0000') + string(hour(now()),'00') + string(minute(now()),'00') + & 
	string(second(now()),'00') + ".txt" 
	
//On y transfert les données
	//Préparer les datastores
...
IF ll_nbligne > 0 THEN
	lds_t_recolte_112.SaveAs(gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", SQLInsert!, FALSE)
	ls_string = ""
	IF FileExists(gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt") THEN
		li_FileNum = FileOpen(gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", TextMode!)
		IF li_FileNum = -1 THEN
			gnv_app.inv_error.of_message("CIPQ0151",{"Ouverture de fichier", gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", "1 of_transfert recolte (inter-centre)"})
		END IF		
		FileReadEX(li_FileNum, ls_string)
		FileClose(li_FileNum)
		...
		LOOP
	ELSE
		gnv_app.inv_error.of_message("CIPQ0153", {gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", "of_transfert_recolte_commande 1" })
	END IF
	ls_ecrire = ls_ecrire + "~r~n" + ls_string + " "
...
IF ll_nbligne > 0 THEN
	lds_t_recolte_commande_112.SaveAs(gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", SQLInsert!, FALSE)
	ls_string = ""
	IF FileExists(gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt") THEN
		li_FileNum = FileOpen(gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", TextMode!)
		IF li_FileNum = -1 THEN
			gnv_app.inv_error.of_message("CIPQ0151",{"Ouverture de fichier", gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", "2 of_transfert recolte_commande (inter-centre)"})
		END IF		
		FileReadEX(li_FileNum, ls_string)
		FileClose(li_FileNum)
		...
		LOOP
	ELSE
		gnv_app.inv_error.of_message("CIPQ0153", {gnv_app.of_getPathDefault()+gnv_app.of_getODBC()+"\tempo.txt", "of_transfert_recolte_commande 2" })
	END IF
	ls_ecrire = ls_ecrire + "~r~n" + ls_string + " "
...
IF trim(ls_ecrire) <> "" AND TRIM(gnv_app.inv_string.of_globalreplace( ls_ecrire, "~r~n", "")) <> "" THEN
	...
	IF ll_rtn < 0 THEN
		gnv_app.inv_error.of_message("CIPQ0151",{"Écriture du fichier de transfert", ls_repexport + "TMP_" + ls_nomfichier, "of_transfert_recolte_commande (inter-centre)"})
	ELSE
		//Renommer le fichier
		ll_rtn = gnv_app.inv_filesrv.of_filerename( ls_repexport + "TMP_" + ls_nomfichier, ls_repexport + ls_nomfichier)
		IF ll_rtn < 0 THEN
			gnv_app.inv_error.of_message("CIPQ0151",{"Écriture du fichier de transfert - rename", ls_repexport + "TMP_" + ls_nomfichier, "of_transfert_recolte_commande (inter-centre)"})
		END IF
	...
	//Validation pour la réceptions du fichier des récoltes
	INSERT INTO t_TransfertCommande ( CieNo, DateCommande, NomFichier, HeureTransfert )
	VALUES ('111', :ldt_cur, :ls_nomfichier, :ldt_cur);
	IF SQLCA.SQLCode <> 0 then
		gnv_app.inv_error.of_message("CIPQ0152",{"Validation pour la réceptions du fichier des récoltes", SQLCA.SQLeRRText})
	ELSE
		COMMIT USING SQLCA;
		IF SQLCA.SQLCode <> 0 then
			gnv_app.inv_error.of_message("CIPQ0152",{"Validation pour la réceptions du fichier des récoltes", SQLCA.SQLeRRText})
	...
	IF LEFT(Upper(lnv_dirlistattrib[ll_cpt].is_filename),3) <> "TMP" and LEFT(Upper(lnv_dirlistattrib[ll_cpt].is_filename),3) <> 'ZIP' THEN
		...
		CHOOSE CASE UPPER(MID(lnv_dirlistattrib[ll_cpt].is_filename, 4,1))
			CASE "C" //Fichier de commande
				...
				// Vérifier si on a l'entièreté du fichier
				...
				if isNumber(ls_ligne[1]) then
					if gnv_app.inv_filesrv.of_getFileSize(ls_repimport + lnv_dirlistattrib[ll_cpt].is_filename) <> double(ls_ligne[1]) then
						gnv_app.inv_error.of_message("CIPQ0018",{"L'intégrité du fichier de commande ~"" + ls_repimport + lnv_dirlistattrib[ll_cpt].is_filename + "~" est corrompue.~r~n" + &
																			  "Le fichier a été déplacé dans le dossier d'archivage sans être traité."})
					...
					gnv_app.inv_error.of_message("CIPQ0020",{"L'intégrité du fichier de commande ~"" + ls_repimport + lnv_dirlistattrib[ll_cpt].is_filename + "~" n'a pas pu être vérifiée."})
					li_debutboucle = 1
				end if
				...
				if lb_traiter_fichier then
					FOR ll_cpt_ligne = li_debutboucle TO ll_nbligne
						IF ls_ligne[ll_cpt_ligne] <> "" AND NOT ISNULL(ls_ligne[ll_cpt_ligne]) THEN
							...
							//Code pour mettre le nouveau numéro de commande
							IF POS(upper(ls_ligne[ll_cpt_ligne]), "INSERT INTO T_COMMANDE") > 0 THEN
								...
								IF ll_nbligne_valeur > 1 THEN
									...
									//Si c'est le detail prendre le numéro du père
									IF POS(upper(ls_ligne[ll_cpt_ligne]), "INSERT INTO T_COMMANDEDETAIL") > 0 THEN
										// 2008-10-24 changé - ls_newnocommande = ls_dernier_numero_commande
										...
										ls_array_value[8] = "''"
									ELSE
										//C'est le père récupérer le prochain numéro
									...
									IF ll_nbligne_valeur = 25 THEN //INSERT INTO t_commande
										...
										// Ralentir d'une seconde pour ne pas que le prochain fichier ait le même nom
										Sleep(1)
										...
										//Déterminer ce qui ira dans le fichier
										IF trim(ls_ecrire) <> "" THEN
											ll_rtn = gnv_app.inv_filesrv.of_filewrite( ls_repexport + "TMP_" + ls_ConfirmationFile, ls_ecrire )
											IF ll_rtn < 0 THEN
												gnv_app.inv_error.of_message("CIPQ0151",{"Écriture du fichier de confirmation de transfert de commande", ls_repexport + "TMP_" + ls_ConfirmationFile, "Importation de fichier (inter-centre)"})
											ELSE
												//Renommer le fichier
												ll_rtn = gnv_app.inv_filesrv.of_filerename( ls_repexport + "TMP_" + ls_ConfirmationFile, ls_repexport + ls_ConfirmationFile)
												IF ll_rtn < 0 THEN
													gnv_app.inv_error.of_message("CIPQ0151",{"Écriture du fichier de confirmation de transfert de commande - rename", ls_repexport + ls_ConfirmationFile, "Importation de fichier (inter-centre)"})
												END IF
											END IF
										END IF
							...
							IF TRIM(ls_ligne[ll_cpt_ligne]) <> "" THEN
								...
								IF SQLCA.SQLCode <> 0 then
									gnv_app.inv_error.of_message("CIPQ0152",{"(C) SQL importés du fichier: " + lnv_dirlistattrib[ll_cpt].is_filename, SQLCA.SQLeRRText})
								ELSE
									COMMIT USING SQLCA;
									IF SQLCA.SQLCode <> 0 then
										gnv_app.inv_error.of_message("CIPQ0152",{"(C) SQL importés du fichier: " + lnv_dirlistattrib[ll_cpt].is_filename, SQLCA.SQLeRRText})
			...
			CASE "K", "O"
				//K=confirmation de la réception d'un fichier de commande
				//O=confirmation de la réception d'un fichier de récolte/commande
				
				//Simplement rouler le contenu du fichier
				EXECUTE IMMEDIATE :ls_contenu USING SQLCA;
				IF SQLCA.SQLCode <> 0 then
					gnv_app.inv_error.of_message("CIPQ0152",{"(K,O) SQL importés du fichier: " + lnv_dirlistattrib[ll_cpt].is_filename, SQLCA.SQLeRRText})
				ELSE
					COMMIT USING SQLCA;
					IF SQLCA.SQLCode <> 0 then
						gnv_app.inv_error.of_message("CIPQ0152",{"(K,O) SQL importés du fichier: " + lnv_dirlistattrib[ll_cpt].is_filename, SQLCA.SQLeRRText})
			...
			CASE "T"
				...
				// 2009-11-25 Sébastien Tremblay - passer par des fichier "Tmp"
				...
				IF FileExists(ls_ReplImpDir) THEN
					...
					IF ll_rtn < 1 THEN
						gnv_app.inv_error.of_message("CIPQ0151",{ls_repimport + lnv_dirlistattrib[ll_cpt].is_filename, ls_repimportold + lnv_dirlistattrib[ll_cpt].is_filename, "Importation de fichier (inter-centre) - Déplacer le fichier dans importold (pilotage)"})
					END IF				
					
					do while yield()
					loop
					
					// Renommer le fichier temporaire pour enlever le préfixe "Tmp" (la copie est terminée)
					...
					IF FileExists(ls_repimportold + lnv_dirlistattrib[ll_cpt].is_filename) THEN
						...
						IF ll_rtn < 1 THEN
							gnv_app.inv_error.of_message("CIPQ0151",{ls_repimportold + 'Tmp' + lnv_dirlistattrib[ll_cpt].is_filename, ls_repimportold + string(hour(now())) + "_" + string(minute(now())) + "_" + lnv_dirlistattrib[ll_cpt].is_filename, "Téléchargement de fichier (inter-centre) - Renommer le fichier dans importold (pilotage)"})
						END IF
					ELSE
						...
						IF ll_rtn < 1 THEN
							gnv_app.inv_error.of_message("CIPQ0151",{ls_repimportold + 'Tmp' + lnv_dirlistattrib[ll_cpt].is_filename, ls_repimportold + lnv_dirlistattrib[ll_cpt].is_filename, "Téléchargement de fichier (inter-centre) - Renommer le fichier dans importold (pilotage)"})
						END IF
					END IF
					...
					IF ll_rtn < 0 THEN
						gnv_app.inv_error.of_message("CIPQ0151",{ls_repimport + lnv_dirlistattrib[ll_cpt].is_filename, ls_ReplImpDir + 'Tmp' + lnv_dirlistattrib[ll_cpt].is_filename, "Téléchargement de fichier (inter-centre) - Déplacer le fichier dans recoie (pilotage)"})
					END IF
					...
					IF ll_rtn < 0 THEN
						gnv_app.inv_error.of_message("CIPQ0151",{ls_ReplImpDir + 'Tmp' + lnv_dirlistattrib[ll_cpt].is_filename, ls_ReplImpDir + ls_nom_fichier, "Téléchargement de fichier (inter-centre) - Renommer le fichier dans recoie (pilotage)"})
					END IF
				ELSE
					gnv_app.inv_error.of_message("CIPQ0150")
				END IF
				
			Case "R" //Fichier de Récolte_Commande
				...
				EXECUTE IMMEDIATE :ls_contenu USING SQLCA;
				IF SQLCA.SQLCode <> 0 then
					gnv_app.inv_error.of_message("CIPQ0152",{"(R) SQL importés du fichier: " + lnv_dirlistattrib[ll_cpt].is_filename, SQLCA.SQLeRRText})
				ELSE
					COMMIT USING SQLCA;
					IF SQLCA.SQLCode <> 0 then
						gnv_app.inv_error.of_message("CIPQ0152",{"(R) SQL importés du fichier: " + lnv_dirlistattrib[ll_cpt].is_filename, SQLCA.SQLeRRText})
				...
				IF trim(ls_ecrire) <> "" THEN
					ll_rtn = gnv_app.inv_filesrv.of_filewrite( ls_repexport + "TMP_" + ls_ConfirmationFile, ls_ecrire )
					IF ll_rtn < 0 THEN
						gnv_app.inv_error.of_message("CIPQ0151",{"Écriture du fichier de confirmation des récoltes-commandes", ls_repexport + "TMP_" + ls_ConfirmationFile, "Importation de fichier (inter-centre)"})
					ELSE
						//Renommer le fichier
						ll_rtn = gnv_app.inv_filesrv.of_filerename( ls_repexport + "TMP_" + ls_ConfirmationFile, ls_repexport + ls_ConfirmationFile)
						IF ll_rtn < 0 THEN
							gnv_app.inv_error.of_message("CIPQ0151",{"Écriture du fichier de confirmation des récoltes-commandes - rename", ls_repexport + ls_ConfirmationFile, "Importation de fichier (inter-centre)"})
						END IF
					END IF
				END IF
		...
		//Déplacer le fichier dans importold
		...
		If UPPER(MID(lnv_dirlistattrib[ll_cpt].is_filename, 4,1)) <> "T" THEN
			if FileExists(ls_repimportold + lnv_dirlistattrib[ll_cpt].is_filename) THEN
				if FileDelete(ls_repimportold + lnv_dirlistattrib[ll_cpt].is_filename) then
					...
					IF ll_rtn < 1 THEN
						gnv_app.inv_error.of_message("CIPQ0151",{ls_repimport + lnv_dirlistattrib[ll_cpt].is_filename, ls_repimportold + lnv_dirlistattrib[ll_cpt].is_filename, "Importation de fichier (inter-centre) - Déplacer le fichier dans importold (à la toute fin)"})
					END IF
				else
					gnv_app.inv_error.of_message("CIPQ0151",{ls_repimport + lnv_dirlistattrib[ll_cpt].is_filename, ls_repimportold + lnv_dirlistattrib[ll_cpt].is_filename, "Importation de fichier (inter-centre) - Essayer de fermer CIPQ Transfert et réouvrir Si l'erreur persiste, déplacer le fichier dans importold (à la toute fin)"})
				end if
			else
				ll_rtn = FileMove(ls_repimport + lnv_dirlistattrib[ll_cpt].is_filename, ls_repimportold + lnv_dirlistattrib[ll_cpt].is_filename)
				IF ll_rtn < 1 THEN
...
//Supprime tous les fichiers dont le FileDateTime est < à 15 jours de la date du jour
