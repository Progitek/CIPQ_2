$PBExportHeader$w_gestion_lot.srw
forward
global type w_gestion_lot from w_sheet_frame
end type
type uo_fin from u_cst_toolbarstrip within w_gestion_lot
end type
type st_1 from statictext within w_gestion_lot
end type
type em_date from u_em within w_gestion_lot
end type
type pb_go from picturebutton within w_gestion_lot
end type
type rb_melange from u_rb within w_gestion_lot
end type
type rb_pure from u_rb within w_gestion_lot
end type
type st_2 from statictext within w_gestion_lot
end type
type em_expiration from u_em within w_gestion_lot
end type
type st_3 from statictext within w_gestion_lot
end type
type st_4 from statictext within w_gestion_lot
end type
type dw_gestion_lot_famille from u_dw within w_gestion_lot
end type
type dw_gestion_lot_lot from u_dw within w_gestion_lot
end type
type cb_calcul from commandbutton within w_gestion_lot
end type
type cb_rec_dispo from commandbutton within w_gestion_lot
end type
type dw_gestion_lot_lot_detail from u_dw within w_gestion_lot
end type
type uo_gauche from u_cst_toolbarstrip within w_gestion_lot
end type
type uo_bas from u_cst_toolbarstrip within w_gestion_lot
end type
type uo_mid from u_cst_toolbarstrip within w_gestion_lot
end type
type uo_droite from u_cst_toolbarstrip within w_gestion_lot
end type
type gb_1 from groupbox within w_gestion_lot
end type
type gb_3 from groupbox within w_gestion_lot
end type
type gb_4 from groupbox within w_gestion_lot
end type
type rr_1 from roundrectangle within w_gestion_lot
end type
type ddlb_extension from u_ddlb within w_gestion_lot
end type
type dw_gestion_lot_produit from u_dw within w_gestion_lot
end type
type gb_2 from groupbox within w_gestion_lot
end type
type dw_gestion_lot_produit_verrat from u_dw within w_gestion_lot
end type
type ole_com from olecustomcontrol within w_gestion_lot
end type
end forward
...
global w_gestion_lot w_gestion_lot
...
IF THIS.event pfc_save() >= 0 THEN
	IF gnv_app.inv_error.of_message("CIPQ0097") = 1 THEN
		...
		ls_StringToSend = ls_boardid + space (16 - len(ls_boardid)) + space(10) + ls_batchid + Space(10 - Len(ls_BatchID)) + &
							...
							space(12 - len(ls_regnumber)) + space(6) + ls_date + &
							Space(8 - Len(ls_date)) + space(2) + string(ll_tubetopackage) + &
...
//		Messagebox("Ligne", ls_stringtosend)
//		RETURN
		//Ouverture du port
		ole_com.object.Settings = "2400,N,8,1"
		ole_com.object.InputLen = 0
...
this.Control[iCurrent+25]=this.dw_gestion_lot_produit
this.Control[iCurrent+26]=this.gb_2
this.Control[iCurrent+27]=this.dw_gestion_lot_produit_verrat
this.Control[iCurrent+28]=this.ole_com
end on

on w_gestion_lot.destroy
call super::destroy
...
type st_title from w_sheet_frame`st_title within w_gestion_lot
integer x = 229
string text = "Gestion des lots de récoltes"
end type

type p_8 from w_sheet_frame`p_8 within w_gestion_lot
integer x = 41
integer y = 20
integer width = 160
integer height = 132
...
type rr_infopat from w_sheet_frame`rr_infopat within w_gestion_lot
integer x = 9
integer y = 16
integer width = 4562
integer height = 140
...
boolean focusrectangle = false
string binarykey = "w_gestion_lot.win"
integer textsize = -10
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
