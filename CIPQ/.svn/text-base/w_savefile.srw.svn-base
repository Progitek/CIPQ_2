HA$PBExportHeader$w_savefile.srw
forward
...
setRedraw(false)
	...
	if fileExists(is_corrpath) then
		if gnv_app.of_getLangue() = 'an' then ls_a = ' at ' else ls_a = ' $$HEX2$$e0002000$$ENDHEX$$'
		...
		For i = 1 To ll_nb
			if left(lo_dirList[i].is_FileName,1) <> '.' then
		   	...
		   	setitem(ll_newrow,'pfile',"Z:\CIPQ\images\file\file-doc.png")
		...
		For i = 1 To ll_nb
			if left(lo_dirList[i].is_FileName,1) <> '.' then
				...
				setitem(ll_newrow,'pfile',"Z:\CIPQ\images\file\file-pdf.png")
		...
		For i = 1 To ll_nb
			if left(lo_dirList[i].is_FileName,1) <> '.' then
				...
				setitem(ll_newrow,'pfile',"Z:\CIPQ\images\file\file-jpg.png")
			end if
		Next
		...
		For i = 1 To ll_nb
			if left(lo_dirList[i].is_FileName,1) <> '.' then
				...
				setitem(ll_newrow,'pfile',"Z:\CIPQ\images\file\file-txt.png")
		...
		For i = 1 To ll_nb
			if left(lo_dirList[i].is_FileName,1) <> '.' then
				...
				setitem(ll_newrow,'pfile',"Z:\CIPQ\images\file\file-xls.png")
